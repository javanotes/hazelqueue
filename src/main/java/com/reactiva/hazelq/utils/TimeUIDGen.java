...
//Copied from Cassandra source.

/**
 * The goods are here: www.ietf.org/rfc/rfc4122.txt.
 */
public class TimeUIDGen
{
    ...
    private static final TimeUIDGen instance = new TimeUIDGen();
    ...
    private static Collection<InetAddress> getAllLocalAddresses() {
      ...
      try 
      {
        ...
        if(nets != null)
        {
          ...
          {
            Enumeration<InetAddress> inets = nets.nextElement().getInetAddresses();
            if(inets != null)
            {
              while(inets.hasMoreElements())
              {
                list.add(inets.nextElement());
              }
            }
          }
        }
